// Service worker (MV3) — generated into background.js by WXT.
//
// The UI lives in a Side Panel (entrypoints/sidepanel). Clicking the toolbar
// icon opens the panel; the panel itself injects the engine (engine.js) into
// the active tab and drives it over messaging, and persists the per-site
// editing session in chrome.storage.local. The only jobs left here are:
...
//
// NOTE: with WXT, all runtime code must live inside the defineBackground
// callback — the module itself is imported in a Node context at build time.
